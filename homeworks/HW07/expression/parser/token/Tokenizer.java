...
public class Tokenizer {
    ...
    private void tokenize() throws IncorrectSymbolException {
        for (int i = 0; i < length; i++) {
            ...
            switch(c) {
                case 'l':
                    if (i + 5 <= length && expr.substring(i, i + 5).equals("log10")) {
                        tokens.add(new Token("log10", Token.Type.LOG10, i + 1));
                    }
                    i += 4;
                    break;
                case 'p':
                    if (i + 5 <= length && expr.substring(i, i + 5).equals("pow10")) {
                        tokens.add(new Token("pow10", Token.Type.POW10, i + 1));
                    }
                    i += 4;
                    break;
                case '*':
